...
export default function OTPInput({ email, optLength }) {
  ...
  async function optResend() {
    // eslint-disable-next-line
    let data = await axios.post(
      `${RegisterRoute}/resend`,
      {},
  ...
  async function otpSubmit() {
    if (otp === "" || otp === " ") {
      messageParagraph.current.innerHTML = "Please enter a code ";
    ...
    if (!otp || otp !== " " || otp !== "") {
      ...
      if (data.data.status === 400) {
        messageParagraph.current.innerHTML = data.data.message;
        alert.current.style.display = "flex";
      } else {
    ...
    <div className={style.page}>
      ...
      <div className={style.container}>
        <div className={style.modal}>
          <h3 className={style.heading}>Enter security code</h3>

          <div className={style.belowHeading}>
            <div ref={alert} className={style.alert}>
              <span className={style.imgContainer}>
                <img
                  src="https://static.xx.fbcdn.net/rsrc.php/v3/yC/r/YSnPoUWKaq9.png"
                  alt=""
                />
              </span>
            ...
            <span className={style.information}>
              ...
              {isEmailPresent && (
                <span>
                  <p className={style.emailMessage}>We sent your code to:</p>
                  <p className={style.email}>{email ? email : "Your email"}</p>
                </span>
              )}
            </span>
          </div>
          <hr className={style.hr} />
          <span className={style.buttonGrid}>
            <button className={style.resend} onClick={optResend}>
              Didn't get a code?
            </button>
            <span className={style.rightButton}>
              <button className={style.cancel} onClick={otpFormCloser}>
                Cancel
              </button>
              <button
                ref={submitButton}
